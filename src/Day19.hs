...
fitScanner fitted ((s, ps):ss) tried =
  let
    fits = do
      ...
      let off1 = (x0-x1,y0-y1,z0-z1)
      -- offset the points we're trying to fit
      let poff' = Set.map (offset3d off1) ps'
      let ints = Set.intersection p0s poff'
      guard $ Set.size ints >= 12
